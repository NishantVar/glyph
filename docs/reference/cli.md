# Glyph CLI — Reference (v0)

This is the stable contract for the `glyph` binary. Tools, agents, and scripts may depend on the behavior described here. For the high-level product/UX framing, see [[design/cli]]. For rationale on the agent-oriented exit-code scheme, see [[0008-agent-oriented-exit-codes]].

The compiler is **fully deterministic**. LLM-assisted phases — Phase 3 (Repair) and Phase 6 Step 2 (Expand reshaping) — are not part of the compiler binary. They are implemented as an external agent skill that invokes the CLI and handles LLM work between invocations.

## Subcommands

### `glyph compile <path>`

Runs the compiler's deterministic phases: Parse (1), Analyze (2), Lower (4), Validate (5), Expand Step 1 (6-Step1), and Emit (7). Produces compiled `.md` files and optionally `.ir.json` sidecar files.

The compiler does **not** run Phase 3 (Repair) or Phase 6 Step 2 (Expand reshaping). If Phase 2 produces `repairable` diagnostics, the compiler stops after Phase 2 and exits with code 2. If Phase 2 is clean, the compiler continues through the remaining deterministic phases to produce output.
...
Runs Phases 1 (Parse) and 2 (Analyze) only. Reports all diagnostics — errors, repairable, and warnings — without continuing to Lower, Validate, Expand, or Emit. No output files are produced.

If the source passes `check` with exit 0, `compile` will proceed past Phase 2 (though post-Lower Phase 5 validation can still catch rare invariant violations).

Accepts the same `<path>` semantics as `compile` (file or directory, DAG closure).

### `glyph validate-output <ir-json-path> <md-path>`

Runs the deterministic Phase 6b structural checks against Step 2 output. Takes the resolved IR JSON (`foo.ir.json` from `--emit-ir`) and the agent-rewritten Markdown (`foo.md`) as positional arguments. Validates section shape, role preservation (step/constraint counts and ordering), parameter reference integrity, procedure section correctness, and content shape constraints.
...
Runs Phase 3a (deterministic source rewrites) only. No LLM, no IR construction, no compiled output. Rewrites the `.glyph` source files in place.

`fmt` runs in **two strata**:

1. **Pre-Parse text-level rewrites.** Operate on raw source text without an AST.
...
If Phase 1 succeeds (after the pre-Parse pass), both strata run and the file is rewritten. If Phase 1 fails after the pre-Parse pass, `fmt` emits the parse diagnostic and writes only the pre-Parse text fixes (if any); it does not perform the AST-level rewrites.

**`fmt` does not reorder sections.** Sub-section order on disk is the author's source order; the compiler places each section at the canonical body position via the merge algorithm in [[language-surface]]. The recommended source order — which `fmt` preserves but does not enforce — is `description → effects → goal → constraints → context → flow`; authors who follow it get compiled body order matching the default.
...
| `--enable-effects` | | Enable the effects subsystem (parsing, inference, validation, output emission). Default: **off**. When off, `effects:` sub-sections are rejected at parse time, all effect-related diagnostics are suppressed, and the `effects` frontmatter field is omitted from compiled output. |
...
For the rationale behind this scheme, see [[0008-agent-oriented-exit-codes]].

## IR JSON Output

The `--emit-ir` flag on `compile` outputs the **post-Step-1 resolved IR** as a JSON file (`foo.ir.json`) alongside the compiled `.md`. This is the IR after Expand Step 1 (deterministic resolution); see [[ir-json]] for the canonical shape.

The JSON uses a **nested tree** shape (children inlined under parents) rather than a flat arena dump. Each node carries its `node_id` as an attribute.

The companion agent consumes the IR JSON to perform Expand Step 2: it reads the resolved IR for full structural context — including `with` modifiers, roles, and constraint attributes — performs LLM prose reshaping, and writes the final polished `.md`.

**Library files produce no IR JSON.** The IR JSON envelope is rooted in `Skill` per [[ir-json]], and a library file (zero `skill` declarations, only blocks/text/imports) has no Skill to root the envelope on. `--emit-ir` is therefore a silent no-op for IR on library files: the compiler does not write `foo.ir.json`, and the stdout NDJSON wrapper still emits a `{"file": ..., "diagnostics": [], "emitted": [...]}` line listing any procedure `.md` artifacts produced.
...
Output uses **NDJSON** (newline-delimited JSON): one complete JSON object per line, no top-level array wrapper, line-buffered to stdout. Files emit in **topological compile order**, matching the order the compiler processes them in. The agent reads stdout incrementally without waiting for build completion.
...
The `diagnostics` array matches the `Diagnostic` shape defined in [[docs/reference/diagnostics]]. The `emitted` array lists output paths produced for that file (compiled `.md`, `.ir.json` if `--emit-ir`, procedure `.md` files); it is empty when the file did not progress past Phase 2.
...
2. Files are processed in topological order per [[compiler-pipeline]] §Multi-File Compilation Order.
3. Partial failure follows [[compiler-pipeline]] §Partial Failure Policy: failed files skip their dependents, successful files still emit output.
...
- **Incremental compilation** — the pipeline supports it architecturally but v0 re-runs all phases on every invocation.
...
- **Embedded LLM mode** — v0 compiler is fully deterministic. LLM phases (Repair, Expand Step 2) live in an external agent skill.

## Cross-References

- **Pipeline:** [[compiler-pipeline]] — phase definitions, multi-file order, partial failure, cacheability.
- **Diagnostics:** [[docs/reference/diagnostics]] — diagnostic shape, classification, ID scheme.
- **Repair:** [[design/repair]] — repair sub-steps (3a/3b/3c), LLM calls, convergence.
- **Compiled output:** [[design/compiled-output]] — file naming, procedure subdirectories, output structure.
- **IR JSON schema:** [[ir-json]] — JSON shape for `--emit-ir` output, resolved IR fields.
- **Agent skill:** [[agent-skill]] — agent workflow, repair guidance, Step 2 rules, `validate-output` integration.
- **Expand:** [[design/expand]] — Phase 6b validation gate, structural diagnostic IDs used by `validate-output`.
