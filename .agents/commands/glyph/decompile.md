...
1. Decompile by semantic content, not by Markdown shape. Authors write meaning in whatever packaging they have on hand — your job is to recover the meaning into Glyph constructs (block, if, flow step, parameter, constraint, context, description, return). UNMAPPED is a last resort, not a fallback for unfamiliar Markdown shapes.
...
3. Classify each unplaced unit of prose by meaning, not by Markdown shape: `"When X…"` / `"If you need Y…"` / `"For the case where…"` / `"Whenever Z…"` prose, especially when it heads a sub-section, becomes a branch — pick the minimal form (a `block` with `description:` dispatched by `if PROCNAME.applies()` when the arm has a substantive body; an inline string predicate `if "the predicate prose": single_action()` when the arm is a single action and the predicate appears nowhere else; a `const some_predicate = "the prose"` dispatched with `if some_predicate` when the predicate recurs). A sequence of imperative actions under any header becomes a `block` with a `flow:` body, called from the parent flow. `"Always X"` / `"Must X"` / `"Never Y"` / `"Avoid Y"` / `"Prefer Z"` / `"Consider Z"` becomes a constraint marker, polarity from the verb (`Always`/`Must` → `must`/`require`; `Never`/`Avoid` → `must avoid`/`avoid`; `Prefer`/`Consider` → soft `require`). The taxonomy is exhaustive: content that is neither an instruction nor a constraint belongs in `context:`, including reference material (code snippets, command listings, tables, diagrams, worked examples, format specs) embedded as a triple-quoted block-string const — except a source template containing a literal triple-quote sequence, which stays UNMAPPED. A `## Variables` / `## Configuration` / `## Settings` block of user-editable knobs becomes skill parameters with defaults. `"EXAMPLES: …"` / `"e.g. X, Y, Z"` tied to a triggered behavior folds into the trigger block's `description:` string. `"This produces / returns / outputs X"` at the end of a flow becomes `return <"X">` (or `-> DomainType` if a domain noun is named). An import from another skill or shared library becomes an `import` declaration at the top of the file. Truly UNMAPPED is a narrow exception reserved for harness/loader metadata the executing agent never reads (`trigger`, `allowed-tools`, `user-invocable`).
