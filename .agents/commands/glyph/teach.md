...
3. Write the declaration header: for `skill`, parameters may omit defaults (the agent extracts them from user context at runtime), but every `export block` parameter requires one. For return types, use a named domain type (`Plan`, `BranchName`, `Diagnosis`) or omit `->` entirely when there is no meaningful return value — never `String`/`Int`/`Float`/`Bool`/`None`.
4. Add `description:` as a single-line routing string, or as a bare-name reference to a same-file `const` constant — never with a parameter slot inside. Choose constraints next: `require` / `avoid` for soft rules, `must` / `must avoid` for genuinely non-negotiable ones, each marker carrying a bare-name reference to a same-file `const` or an inline string. Add `context:` entries for background facts the agent should keep in mind — bare-name references to string-valued constants, inline strings, or `context`-prefixed markers.
5. Write `flow:` as an ordered sequence of instruction strings, bindings, calls (bare, qualified, or UFCS), and branches — using `with "..."` to specialize a single call site and `if`/`elif`/`else` with `BLOCKNAME.applies()` for description-driven dispatch. Reference parameters and local bindings inside instruction strings with curly-brace parameter slots, but only where the surrounding string is instruction-bearing. End `flow:` with at most one top-level `return`, placed last and never inside a branch arm; on `export block`, that `return` must be explicit even when returning `none`.
...
7. Scan every instruction string in `flow:` bodies; for any longer than 10 words, extract it into a named `block` (or `export block` if it must be reachable from another file) and replace the inline string with a call, picking a verb-phrase name that describes the step's intent. Then scan every inline string used as a marker body (`require`/`avoid`/`must`/`must avoid`/`context`) or as a `context:` entry — for any longer than 10 words, extract it into a named `const` constant (or `export const` if another file imports it) and replace the inline string with a bare-name reference. Skip `description:` strings — leave them inline.
...
9. Run the Glyph compiler on {target} and read the diagnostics; on repairable diagnostics (exit 2), run `glyph fmt` on {target} and re-invoke the compiler if the file changed. Treat errors as required fixes and warnings as advisory; remaining repairable diagnostics after `glyph fmt` will be handled by the LLM repair pass, which may insert `generated const` / `generated block` definitions, hoist markers, or fill missing descriptions. Review the source diff to promote anything you want to harden by renaming `generated …` to `const` / `block`, iterate until the file compiles cleanly with the intended structure, and report the path to the authored or updated `.glyph` file.
...
- You must never build strings with `${...}`, `+` concatenation, or any other template syntax — the only template-like form is curly-brace parameter slots, legal solely inside instruction-bearing strings.
- You must never write the agent-facing Markdown prose by hand — the Expand pass produces the prose, while the author writes structure and intent only.
- You must ensure every skill file contains exactly one `skill` declaration; library files contain zero, and two in one file is a hard error.
...
- You must give every parameter on an `export block` a default value, since a required-without-default parameter is a hard error with no LLM repair.
- You must end every `export block` with an explicit `return` statement (use `return none` if there is no meaningful return value).
- You must never place `return` inside an `if`/`elif`/`else` branch arm — there is no early return in MVP, so `return` is restricted to the top level of `flow:` and must be last.
- You must never use curly-brace parameter slots in `description:` bodies, parameter defaults, or any string that is not instruction-bearing — slots are legal only inside `flow:` instruction strings, string-valued constant bodies, constraint texts, and string arguments to stdlib calls.
- Reach for the smallest viable surface — `skill`, `require`/`avoid`, `flow:`, inline strings, calls, and `with` — and promote to `block`, named `const`, or types only when they pay for themselves.
...
- Ensure any block consulted via `BLOCKNAME.applies()` carries a `description:` (the predicate the agent matches against current context); a missing description on an imported block is a hard error.
- After running the compiler, review the source diff: repair may have inserted `generated const` / `generated block` definitions, hoisted markers into `constraints:`, or generated a missing `description:`, so promote anything you want to harden by renaming `generated …` to `const` / `block`.
...
- Avoid reaching for `must` / `must avoid` for everyday rules — reserve hard markers for genuinely non-negotiable constraints and default to soft `require` / `avoid`.
- Avoid stacking nested calls (`apply(merge(base, overlay(ctx)))`) — they are legal but read and visualize better as named intermediate bindings.
