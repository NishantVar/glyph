...
- Compare the two files by meaning, not by surface form. Equivalent intent with different surface phrasing, different identifier names, or different factoring choices is not a semantic difference. Only items whose semantic content has shifted count.
- Report every semantic difference produced by the cross-read pass under the appropriate heading. A difference left unreported defeats the purpose of the round-trip equivalence check.
- You must never modify, rewrite, or normalize either the original or the roundtrip Glyph file. The equivalence review is read-only — both files stay on disk untouched.
