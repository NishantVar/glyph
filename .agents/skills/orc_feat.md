...
- **prd**. Required. The PRD reference (file path or GitHub issue identifier) whose end-to-end implementation you are orchestrating; a single issue is accepted as a PRD-of-one.
...
4. Create a new branch from main whose name is deterministically derived from the PRD identifier at {prd}, then create a sibling worktree on that branch and capture its absolute path. Refer to this result as worktree.
5. Create a new team that will host the planner and every implementer for this PRD. Refer to this result as team.
6. Spawn one planner teammate inside the team created in the previous step with the worktree from step 4 as its working directory, initialized from the planner agent definition at .claude/agents/planner.md. In the planner's spawn prompt give it: the worktree path, the PRD reference {prd}, and your own team-lead teammate identifier so the planner can SendMessage you back. Refer to this result as planner.
7. Follow the coordinate-with-planner procedure below.
...
9. Push the branch in the worktree from step 4 to the remote, open one pull request to main containing every change for the PRD at {prd} using a title and body derived from the PRD itself, then tell the human the PR is ready, include the PR URL, and note that worktree cleanup is the human's call after the PR is merged.

### Procedure: coordinate-with-planner

1. Wait for the next SendMessage from the planner.
2. Decide which of the following applies and follow only that path:
   ...
   a. Spawn a new implementer teammate inside the team with the worktree as its working directory, initialized from the implementer agent definition at .claude/agents/implementer.md; in its spawn prompt give it the full issue text the planner supplied, the worktree path, and the planner's teammate identifier so the implementer can SendMessage the planner with updates and questions; then SendMessage the new implementer's teammate identifier back to the planner so the planner can address it.
