...
fn ast_rewrite(source: &str, file: &crate::ast::SourceFile) -> String {
    ...
    // Find declaration header lines (indent 0, starts with skill/block/export/const/generated/import).
    ...
    for (i, line) in lines.iter().enumerate() {
        ...
        if !line.starts_with(' ') && !line.starts_with('\t') {
            if trimmed.starts_with("skill ")
                ...
                || trimmed.starts_with("generated const ")
                || trimmed.starts_with("generated block ")
    ...
    // For simple declarations (const, generated const, import), just pass through.
    ...
    for (decl_idx, &(start, end)) in decl_ranges.iter().enumerate() {
        ...
        if header.starts_with("const ")
            ...
            || header.starts_with("generated const ")
