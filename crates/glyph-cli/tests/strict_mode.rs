...
// --- Multi-file repairable preservation under unified pipeline ---

/// Multi-file closure: lib compiles clean, entry triggers a Repairable
/// (`G::analyze::missing-description`) from the analyze pass. Without
/// `--strict`, the new aggregator must return exit 2 (not 1, which would
/// be the pre-fix `BuildResult.exit_code`'s coarse 0/1 mapping).
#[test]
fn no_strict_compile_multi_file_repairable_exits_two() {
    let dir = tempfile::tempdir().unwrap();
    let lib = dir.path().join("lib.glyph");
    let entry = dir.path().join("entry.glyph");

    std::fs::write(&lib, "export const x = \"value\"\n").unwrap();
    // No `description:` sub-section → G::analyze::missing-description (Repairable).
    std::fs::write(
        &entry,
        "import \"./lib.glyph\" { x }\n\
         \n\
    ...
    assert_eq!(
        out.status.code(),
        Some(2),
        "expected exit 2 for multi-file repairable closure; stderr: {}",
        String::from_utf8_lossy(&out.stderr)
    );
}

#[test]
fn strict_compile_multi_file_repairable_exits_one() {
    let dir = tempfile::tempdir().unwrap();
    let lib = dir.path().join("lib.glyph");
    let entry = dir.path().join("entry.glyph");
    ...
    assert_eq!(
        out.status.code(),
        Some(1),
        "expected exit 1 with --strict for multi-file repairable closure; stderr: {}",
        String::from_utf8_lossy(&out.stderr)
    );
}
