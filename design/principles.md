...
7. **Keep the core language intentionally small.**
   The MVP source surface should start with four base declaration kinds: `import`, `text`, `block`, and `skill`. `export` is a visibility modifier (`export block`, `export text`) and `generated` is a repair-authorship modifier (`generated text`). Inside those declarations, Glyph should stay focused on constrained primitives such as `flow`, `call`, `if`, constraints, and `return`. `for_each` is deferred beyond the MVP. Additional declaration kinds may be added later only with strong justification.
