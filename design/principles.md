...
7. **Keep the core language intentionally small.**
   The MVP source surface should start with five top-level declarations: `import`, `text`, `export block`, `block`, and `skill`. Inside those declarations, Glyph should stay focused on constrained primitives such as `flow`, `call`, `if`, constraints, and `return`. `for_each` is deferred beyond the MVP. Additional declaration kinds may be added later only with strong justification.
