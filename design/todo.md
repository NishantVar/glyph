...
- **Block `description:` length warning.** Post-MVP linter rule: warn when a block's inline `description:` string exceeds a threshold (suggested ~140 chars or one prose sentence). The `description:` field is metadata for `BLOCKNAME.applies()` selection, not authored prose; long descriptions should be extracted to a `text` declaration and referenced by name (the existing reference grammar already supports this — see `ir-and-semantics.md` §Block Trigger Predicate). Linter only — not an error or repairable, since the compiler cannot judge "too long" mechanically.
