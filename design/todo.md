...
- **Divergent description warnings.** When the same `-> DomainType` appears on multiple blocks with substantially different `<"...">` descriptions, the compiler could warn that the type name may be overloaded. Deferred because descriptions are local to each block's compiled output and do not affect nominal matching in MVP.
