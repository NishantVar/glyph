...
- **Required `export block` parameters.** MVP keeps `export block` parameter defaults mandatory (`G::analyze::missing-param-default`) because export-block parameter slots are filled by callers at compile time and the rule keeps library blocks reliably callable without surprising missing-arg errors at every consumer site. Post-MVP, consider relaxing to allow required export-block parameters — mirroring ordinary function-call semantics, where authors mark a parameter as having no sensible default and force every caller to supply it. The call-site machinery already exists (private blocks use it today; see `data-flow.md` §Argument Passing). The change is one rule + diagnostic relaxation in `language-surface.md` §3.8. Revisit when a real authoring need surfaces.
