...
**Sub-section ordering is permissive.** Inside a `skill`, `block`, or `export block` body, the parser accepts `context:`, `constraints:`, `description:`, `effects:`, `flow:`, and body-level constraint markers (`require`/`avoid`/`must`/`must avoid`) in **any order** — but source order is semantically significant for compiled output position (see §2.5a). Body-level `context <name|string>` markers are accepted at the same body-level position on all three declaration forms (`skill`, `block`, `export block`); their placement rule is identical to body-level constraint markers (see §4.2a). The only structural rule still enforced is that each named sub-section may appear at most once per body; duplicate occurrences are repairable (the deterministic repair pass merges them in source order).

For a `skill`, body-level constraint and `context` markers hoist into the declaration's `## Constraints` / `## Context` H2s per §4.2a. For a private `block`, the same markers stay on the block's IR node (no H2 emission — private blocks have no peer-level H2 sections); when present, they additionally promote the block to Tier 2 at its call site and render as the procedure preamble (see [[docs/reference/compiled-output]] §Three-Tier Block Projection and §Procedure Preamble (Tier 2 and Tier 3)). For an `export block`, body-level markers stay on the parsed AST (`ExportBlockDecl.body_constraints` / `body_context`) and render only as the Tier 3 standalone procedure preamble — they do **not** hoist into `## Constraints` / `## Context` H2s in the standalone `.md`.
