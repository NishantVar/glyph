...
For `skill` and `export block`, body-level constraint and `context` markers hoist into the declaration's `## Constraints` / `## Context` H2s per §4.2a. For a private `block`, the same markers stay on the block's IR node (no H2 emission — private blocks have no peer-level H2 sections); when present, they additionally promote the block to Tier 2 at its call site and render as the procedure preamble (see [[docs/reference/compiled-output]] §Three-Tier Block Projection and §Procedure Preamble (Tier 2 and Tier 3)).
