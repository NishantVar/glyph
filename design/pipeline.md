...
- **Condition-kind classification.** For every `if` / `elif` condition, Analyze classifies each token in the condition expression as one of: `boolean` (Bool-kinded name, dot access, equality result, etc.), `predicate_applies` (`BLOCKNAME.applies()` form), `predicate_const` (bare name resolving to a string-kinded `const`), `predicate_literal` (inline quoted string in condition position), or `operator` (`and` / `or` / `not` / `==` / parens). The classification is attached to the AST node (option A per `ir-and-semantics.md` §Predicates) and threaded forward to Lower. A condition token that resolves to an Int- or Float-kinded declaration emits `G::analyze::condition-non-boolean-non-predicate` (error). Opaque/domain-kinded bindings fall through to boolean treatment (no regression).
...
- **Unresolved bare names** (no parens at use site): generates a `generated const` definition — a single-string expansion — and appends it to the end of the file. **Condition-position routing rule:** an undefined bare name in an `if` / `elif` condition is always routed to `generated const` (not `generated block`), consistent with constraint-marker and context-marker routing. The LLM generates a single-clause predicate string from the name, surrounding flow context, and the enclosing skill's description. See `repair.md` §6 for the full routing table and prompt context.
...
- **Predicate resolution.** For each `Branch` IR node whose `condition` (or any `elif_branches[*].condition`) contains any predicate-form token, Step 1 populates the Branch's `resolved_predicates: {predicate_token → resolved_string}` side-map (per `ir-schema.md` §Resolved IR `ResolvedBranch`, `ir-json-schema.md` §Branch). Resolution by form: (1) `.applies()` — reads the referenced block's `description:` string; (2) string-const — reads the const's body from the resolved IR; (3) inline literal — not stored in the map (literal is already present in the condition string). The condition string itself is preserved unchanged. Step 2 reads the unified map to choose between the pure-predicate "decide which applies" form and the mixed-condition inline form (per `compiled-output.md` §Predicate-Driven Branch Projection).
